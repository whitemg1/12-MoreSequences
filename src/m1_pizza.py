...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 1:  400 red dots.'
    window = rg.RoseWindow(400, 400, title)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    d_angle = (math.pi * 2)/number_of_points

    for k in range(number_of_points):
        x = circle.center.x + circle.radius * math.cos(k * d_angle)
        y = circle.center.y + circle.radius * math.sin(k * d_angle)

        new_circle = rg.Circle(rg.Point(x,y),10)
        new_circle.fill_color = color


        new_circle.attach_to(window)

    point_list = generate_points_on_circle(circle, number_of_points)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  10 slices, thin (thickness=5) sky blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 5
    pizza(window, circle, 10, 'sky blue', 5)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 13, 'green', 10)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(list_of_points)-1):
        point1 = list_of_points[k]
        point2 = list_of_points[k + 1]
        line = rg.Line(point1,point2)
        line.color = color
        line.thickness = thickness
        line.attach_to(window)

    last_line = rg.Line(list_of_points[0],list_of_points[len(list_of_points) - 1])
    last_line.color = color
    last_line.thickness = thickness
    last_line.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(100, 100), 80)
    fancy_polygon(window, circle, 30, 3, 'sky blue', 3)
    window.continue_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(list_of_points)):
        point1 = list_of_points[k]
        point2 = list_of_points[(k + hops_to_next_point) % number_of_lines]
        arrow = rg.Line(point1,point2)
        arrow.color = color
